...
void solve() {   
  int n;cin>>n;
  vector<ll>v;
  deque<ll>q;ll maxx=INT_MIN;ll sum1=0,sum2=0;
  for(int i=1;i<=n;i++){
    cin>>x[i];
    maxx=max(x[i],maxx);
  }
  ...
  // 理论上用队列应该也可以 下面的是wa的代码
//   int flag=0;
//   for(itn i=1;i<=n;i++){
//     if(i==1&&x[i]<0||i==n&&x[i]<0) continue;
//     if(x[i]<0&&q.size()==0){flag=1;break;}
//     if(x[i]<0){
//         int t=q.back();
//         q.pop_back();
//         q.push_back(t+x[i+1]);i++;
//     }else{
//         q.push_back(x[i]);
//     }
//   }int i=1;//ll sum1=0,sum2=0;
//   if(flag==1){
//     cout<<maxx endl;return ;
//   }
//   while(q.size()!=0){
//     if(i%2==0){
//         sum2+=q.front();q.pop_front();
//     }else{
//         sum1+=q.front();q.pop_front();
//     }
//     i++;
//   }
//   if(sum1<=0&&sum2<=0){
